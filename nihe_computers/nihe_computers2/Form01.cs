...
namespace nihe_computers2
{
    public partial class Form01 : Form
    {
        public Form01()
        {
            ...
            this.C1.BSMonitor.Filter = "Deleted = 'false'";
            ...
            this.C1.SimpleDevice.ChangeStringValueEvent += new ManagerChangeStringValueDelegate(ChangeStringValue);
          //  this.C1.SimpleDevice.ChangeListValueEvent += new ManagerChangeStringValueDelegate(ChangeListValue);
        ...
        void Processor_StartEditProcessorTable(object sender, BindingSource BSProcessor)
        {
            ...
            Form08Processor F = new Form08Processor(C1, C1.BSProcessor);
            F.ShowDialog();
            if (F.DialogResult == System.Windows.Forms.DialogResult.OK)
                C1.DS.AcceptChanges();
            else
        ...
        private void compDatagridView_formatting(int RowIndex, int ColumnIndex, List<string> SelectedUser, List<string> SelectedNumbers)
        {
            ...
            switch (ColumnIndex)
            {
                ...
                case 6:
                    ...
                    if (cell.Value as string == null || cell.Value as string == "")
                    {
                        cell.ErrorText = "Неиспользуемый компьютер";
                    }
                    else
                    {
                        cell.ErrorText = string.Empty;
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                case 0:
                    this.C1.BSComp.Filter = string.Format("Num Like '%{0}%'", textBox1.Text);
                    break;
                case 1:
                    //this.C1.BSMonitor.Filter = string.Format("Number Like '%{0}%'", textBox1.Text);
                    break;
                default:
                    break;
            }
